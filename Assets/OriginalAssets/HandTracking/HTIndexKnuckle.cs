...
public class HTIndexKnuckle : MonoBehaviour
{
    //表示するオブジェクトを定義
    ...
    void Start()
    {
        handjoint = GameObject.Find("IndexFinger");
    }
    ...
    void Update()
    {
        ...
        //該当する関節の位置情報を取得
        if (HandJointUtils.TryGetJointPose(TrackedHandJoint.IndexKnuckle, Handedness.Right, out MixedRealityPose pose1) && HandJointUtils.TryGetJointPose(TrackedHandJoint.IndexMiddleJoint, Handedness.Right, out MixedRealityPose pose2))
        {
