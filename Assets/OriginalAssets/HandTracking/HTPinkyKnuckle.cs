...
public class HTPinkyKnuckle : MonoBehaviour
{
    //表示するオブジェクトを定義
    ...
    void Start()
    {
        handjoint = GameObject.Find("LittleFinger");
    }
    ...
    void Update()
    {
        ...
        //該当する関節の位置情報を取得
        if (HandJointUtils.TryGetJointPose(TrackedHandJoint.PinkyKnuckle, Handedness.Right, out MixedRealityPose pose1) && HandJointUtils.TryGetJointPose(TrackedHandJoint.PinkyMiddleJoint, Handedness.Right, out MixedRealityPose pose2))
        {
