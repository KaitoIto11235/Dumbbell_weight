...
public class HTMiddleKnuckle : MonoBehaviour
{
    //表示するオブジェクトを定義
    ...
    void Start()
    {
        handjoint = GameObject.Find("MiddleFinger");
    }
    ...
    void Update()
    {
        ...
        //該当する関節の位置情報を取得
        if (HandJointUtils.TryGetJointPose(TrackedHandJoint.MiddleKnuckle, Handedness.Right, out MixedRealityPose pose1) && HandJointUtils.TryGetJointPose(TrackedHandJoint.MiddleMiddleJoint, Handedness.Right, out MixedRealityPose pose2))
        {
